    ...
    <>
      ...
      <div className="bg-dark text-white py-16 px-4 text-center">
        ...
        <p className="text-white/60 text-sm">
          <Link href="/" className="hover:text-primary">Home</Link> / About Us
        </p>
      </div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          {/* Image placeholder */}
          <div className="h-96 bg-soft-red flex items-center justify-center text-primary/30 uppercase tracking-widest text-sm rounded-card">
            Organization Photo
          </div>

          <div>
            <p className="text-primary uppercase tracking-widest text-sm font-semibold mb-3">About CARE</p>
            <h2 className="text-3xl font-serif text-dark mb-6 leading-snug">
            ...
            <Link href="/what-we-do" className="btn-primary inline-block">Our Mission</Link>
          </div>
        </div>
      </section>

      {/* Mission & Vision */}
      <section id="mission" className="py-20 bg-off-white">
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={title} className="bg-white p-8 border-t-4 border-primary text-center shadow-card rounded-b-card">
                <h3 className="text-dark font-semibold text-xl mb-4">{title}</h3>
                <p className="text-mid-gray text-sm leading-relaxed">{text}</p>
              </div>
            ))}
          </div>
        </div>

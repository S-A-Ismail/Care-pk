    ...
    <>
      <div className="bg-green text-white py-16 px-4 text-center">
        ...
        <p className="text-white/60 text-sm">
          <Link href="/" className="hover:text-gold">Home</Link> / Join Us
        </p>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
            {[
              ...
              <div key={title} className="card text-center">
                ...
                <h3 className="text-green font-semibold text-xl mb-3">{title}</h3>
                <p className="text-gray-500 text-sm leading-relaxed mb-4">{desc}</p>
                {link && <Link href={link} className="btn-gold inline-block text-xs">Donate Now</Link>}
              </div>
            ))}
          </div>
